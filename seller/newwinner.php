    ...
    <h2 style="font-size: 35px; font-family:argentina; text-align: center;">Winners List</h2>
            <hr>
                <table class="table table-bordered table-hover" style="background-color: white;">
                    ...
                    $i=1;
                        $sql=mysql_query("select * from hostproduct,bid where bid.prod_id=hostproduct.prid and hostproduct.`status`='Expired' and bid.sellerid='$uid' and bid.sellerid=hostproduct.uid group by hostproduct.`prid`") or die(mysql_error());
                        while($rw=mysql_fetch_array($sql)) {
                           $prid=$rw['prid'];
                             $st=mysql_query("SELECT * FROM product where pro_id='$prid'");
                            $rws=mysql_fetch_array($st);
                           $sqlw=mysql_query("select MAX(bidamt) from bid where prod_id='$prid' and sellerid='$uid'") or die(mysql_error());
                            $row=mysql_fetch_array($sqlw);
                            $amt=$row[0];
                            
                            $show_user=mysql_query("SELECT uid,bid_id,biddate FROM bid where bidamt='$amt' and prod_id='$prid' and sellerid='$uid'");
                            $read_user=mysql_fetch_array($show_user);
                            $win_user=$read_user['uid'];
                            $bid_id= $read_user['bid_id'];
                            $date= $read_user['biddate'];
                            ...
                            if($num_rows<=0){
                            $ins_win=mysql_query("INSERT INTO `winnerlist`(`uid`, `bid_id`, `prod_id`, `win_amt`, `datetime`, `status`, `pay_status`) VALUES ('$user_id','$bid_id','$prid','$amt','$date','winner','')");
                            }
                     ?>
